...
A circular `$ref` **MUST NOT** be admitted, in one batch or across operations: an effective form is materialized by inlining the referenced schema, and a cycle has no resolved form to store or serve. The dependency relation is therefore acyclic, and no group of candidates ever has to be admitted atomically (ADR-0012).
