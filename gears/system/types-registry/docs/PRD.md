...
A cycle **MUST NOT** be admitted, in one batch or across operations, and the graph the rule applies to is the **combined** one: `$ref` and derivation edges together. An effective form is materialized by inlining both the referenced schemas and the immediate base, so a cycle over either edge kind — including a mixed one, where a base `$ref`s a schema derived from it — has no resolved form to store or serve. Refusing circular `$ref` alone would leave that mixed cycle admitted. The dependency relation is therefore acyclic, and no group of candidates ever has to be admitted atomically (ADR-0012).
...
- [ ] **Batch admission and startup** — registration and deletion use idempotent asynchronous operations with optimistic candidate preconditions; in-batch references resolve against submitted candidates, independent valid branches may succeed, a circular `$ref` is refused, every candidate receives a keyed outcome, and registry readiness never waits for registrants. (`cpt-cf-types-registry-fr-two-phase-init`)
