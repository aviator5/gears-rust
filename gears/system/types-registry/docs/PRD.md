...
The system **MUST** support platform gear startup registration before the registry is fully ready by staging Admission Candidates without publishing logical entities. Types Registry **MUST** validate the complete required staged startup set, including references between candidates. If validation succeeds, all required initial candidates **MUST** be admitted atomically as `ACTIVE` logical entities with revision `1` before ready state is published. If any required candidate fails validation, none of the staged candidates **MUST** be admitted, the previously committed registry state **MUST** remain unchanged, and ready state **MUST NOT** be published. The failure **MUST** expose sufficient diagnostics for correction and retry.
...
- [ ] The complete required staged startup set is validated and admitted atomically; no staged logical entity becomes ordinarily visible and ready state is not published until every required candidate succeeds.
- [ ] If any required startup candidate fails validation, none of the staged candidates is admitted, the previously committed registry state remains unchanged, ready state is not published, and sufficient diagnostics are available for correction and retry.
