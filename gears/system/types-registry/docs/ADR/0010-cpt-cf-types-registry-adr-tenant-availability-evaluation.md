...
Dependencies still matter for:

* initial admission;
* dependency-revision freshness and revalidation;
* activation of a new base or referenced revision;
* deletion safety and impact analysis.
...
### Option 2: Live resolution dependencies
...
Under the current design direction this could make an Alias depend on its live target while allowing a Type Schema with a materialized effective schema, or a Registered Instance with a stored admitted value, to remain available without their dependencies.

This option makes availability describe the current resolution plan: changing between eager materialization, joins, caches, and on-demand resolution can change which dependencies affect the public verdict.

### Option 3: Live semantic closure

Dependency availability propagates when the target is part of the semantic contract required to use the subject, regardless of whether the target must be physically read during the current request.

Materialization may remove reads from the hot path, but it does not sever semantic dependencies. Disabling a target for a tenant therefore disables every entity that semantically depends on it, transitively.
...
**Every blocking relationship holds between two Managed Entities.** ADR-0011 closes the managed–external boundary in both directions, so no blocking edge crosses it: a Managed Entity has no external target, and an Externally Managed Entity has no managed one. An earlier form of that ADR permitted the second direction and listed it as blocking, which would have left one relationship kind evaluated live by a source while every other was evaluated against managed storage. Removing it leaves one mechanism for one semantic. The availability of an Externally Managed Entity is asserted live by its owning source under ADR-0002 and composes with nothing stored here.
...
P1 has no managed tenant enablement override. Managed deletion safety, ownership-correction checks, admission rules, and the closure of the managed–external boundary may allow an implementation to prove that some blocking targets cannot become unavailable in valid managed P1 state.

An implementation may exploit those invariants and avoid materializing or traversing a closure that cannot affect a P1 verdict. This does not change the selected semantic model: introducing a new availability input later must not change what `AVAILABLE` means or require consumers to reinterpret existing results.
