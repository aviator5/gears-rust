...
* The same GTS Identifier must resolve to the same Registry Reference across processes, deployments, Registry Sources, and data centers without domain gears coordinating ID mappings.
...
* Client and server caches must remain correct in multi-pod and multi-DC deployments and for External Registry Sources.
...
The allocation model is internal to Types Registry, but it must satisfy the public stability contract. The SDK contract does not expose or require a particular UUID version. The current deterministic `GtsId::to_uuid()` approach is the preferred implementation because it naturally satisfies the cross-DC invariant. A different allocation algorithm is permitted only if it preserves all existing mappings and the same global stability guarantees.
...
* the same GTS Identifier always maps to the same UUID across tenants, processes, deployments, Registry Sources, imports, restores, and data centers whenever the caller is authorized to resolve it;
...
* UUID collisions or equal identifiers with different registry definitions are rejected as conflicts;
...
* is bounded by a documented platform maximum;
* is never silently truncated and is not exposed as a sequence of independently usable pages.
...
The response may carry cache or source-freshness metadata, but such metadata does not turn it into an executable plan and cannot replace the concrete UUID members.
...
* Multi-DC compatibility fixtures must pin representative `GTS Identifier <-> UUID` mappings so an implementation or `gts-rust` upgrade cannot silently change persisted identities.
* Cross-DC reconciliation for Managed Entities must compare canonical registry definitions; external-source consistency belongs to the plugin contract.
...
* P1 tests prove that the same GTS Identifier resolves to the same UUID across tenants and simulated data centers; when P2 Alias support is introduced, tests additionally prove that distinct Alias and target identifiers reverse-resolve independently;
* tests cover managed and external logical deletion/tombstone resolution, local-first plugin fallback, invalid external UUID mappings, UUID collision rejection, cross-tenant identifier conflicts, cross-DC managed definition conflicts, tenant availability, bulk operations, wildcard semantics, and cache invalidation;
...
Types Registry can assign a random UUIDv4 once and replicate the `GTS Identifier <-> UUID` mapping to every data center.

* Good, because references are opaque and decoupled from the identifier's spelling.
* Bad, because independent registration of the same GTS Identifier in different data centers can allocate different UUIDs unless allocation is coordinated.
...
* Good, because independent data centers derive the same UUID for the same GTS Identifier without coordinating allocation state.
...
* Whether the Concrete Reference Set contains only currently available references or the domain gear applies its unavailable-entity policy after querying matching stored references.
