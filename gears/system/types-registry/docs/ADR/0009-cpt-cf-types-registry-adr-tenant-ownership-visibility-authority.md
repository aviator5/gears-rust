...
* **one region is outside the grant model entirely.** `gts.cf.toolkit.*` is refused on the tenant plane and cannot be tenant-owned in P1, decided from the candidate's identifier before the PDP is consulted. Under GTS §3.6 the pattern covers the whole derivation chain beneath the prefix, so a type derived from a platform base type and an Instance of either are inside the reservation. This is a bound on what a grant can reach rather than a grant of its own, and it exists because of §Ownership is immutable: the requested owner must equal the family's stored owner, so a platform contract admitted as tenant-owned is not repairable by revision — only by a purge that releases the identifier, which is the more destructive operation ADR-0013 separates for exactly that reason. The reservation is broader than the entities that need it because narrowing it later is additive and touches no stored row, while discovering it should have been broader is not. It also means the corollary above — that a departed tenant's entities have no one left to delete them — cannot arise for a platform contract.
