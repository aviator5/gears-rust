...
Recovering a burned identifier is the whole of the problem. An earlier form of this decision added a second driver — that retained revisions might hold personal data in descriptions, examples, or enum values — and split the mechanism in two to serve it. That driver is withdrawn, and §Non-goals records why.
...
An earlier form of this decision split it in two, adding a content purge that removed retained revisions while keeping the identity tombstone, on the ground that it was production-safe and could serve a personal-data erasure obligation. That half is withdrawn (§Non-goals), and with it the argument for a split: what remains is one act with one risk profile and one guard.
...
Purge requires the entity to be `DELETED`, and re-evaluates the deletion preconditions at execution time: no registered dependent may exist. Under ADR-0011 every dependent is a Managed Entity, so that re-evaluation reads managed storage alone and reaches no plugin. A purge is a mutation and uses the ordinary write path of ADR-0012, so it produces an operation resource, is idempotent, and leaves an audit record.
...
A pattern selects candidates; it does not waive preconditions. The job reports how many entities matched, how many were eligible, and why each of the rest was skipped.
...
Retired Source Claim reservations from ADR-0011 are out of scope. They are also permanent, but releasing one is the business of an explicit claim takeover asserting identity continuity, not of a purge.
...
* ADR-0005 and ADR-0006 are amended: retention is unbounded by policy and bounded only by explicit purge.
* ADR-0001's non-rebinding guarantee acquires one documented exception.
...
* a deleted entity is absent from resolution, discovery, search, and query assistance, while reverse resolution of its Registry Reference still reports that the identifier existed and is deleted;
...
* `cpt-cf-types-registry-fr-id-resolution` - names the single exception to the identifier non-rebinding guarantee and requires it to propagate across datacentres.
* `cpt-cf-types-registry-fr-register-schemas`, `cpt-cf-types-registry-fr-register-instances` - provide the erasure path for personal data retained in admitted revisions.
