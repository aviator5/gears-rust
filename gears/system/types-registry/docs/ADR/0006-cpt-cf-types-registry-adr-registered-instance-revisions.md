...
* Concurrent administrators or gears must not overwrite each other's Instance changes.
...
Types Registry records the exact conforming Type Schema revision on the admitted Instance revision. If that Type Schema revision changes while Instance validation is in progress, the candidate must be revalidated or fail with a structured stale-baseline conflict.
...
Every managed registered Instance content update must carry an `expected_revision` or equivalent strong `ETag`.
...
* the Instance current revision still equals `expected_revision`;
...
A mismatch returns a structured revision conflict and does not activate the candidate. The operation must re-read and revalidate rather than overwrite a concurrent change.
...
Ordinary registered Instance resolution returns at least:

* stable GTS ID and Registry Reference;
* current Instance revision number and content hash or strong ETag;
* current canonical value;
* conforming Type Schema GTS ID and revision used for the current Instance admission;
* lifecycle and tenant availability metadata required by the caller.
...
* `cpt-cf-types-registry-fr-two-phase-init` - defines the per-Instance candidate, initial revision, and publication semantics used by atomic batch admission.
