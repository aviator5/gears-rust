...
This ADR establishes the platform-facing identity policy. The enforced Type Schema Evolution Compatibility mode is decided by ADR-0003. Internal revision representation and retention are decided separately by ADR-0005 and ADR-0006. The lifecycle of the members of a version family is decided by ADR-0008, which was extracted from this ADR so that each document carries one decision.
...
* how the decision amends ADR-0001's same-identifier conflict rule.
...
ADR-0001 states that equal identifiers with different definitions are conflicts. This ADR narrows that rule:

* different concurrent definitions that do not share one admitted revision lineage remain conflicts;
* a stale registrar cannot replace the current revision with an older or divergent definition;
* sequential managed definitions admitted through ADR-0005 or ADR-0006 are valid revisions of the same logical identity and retain the same Registry Reference;
* cross-DC reconciliation compares revision lineage, current revision, and content hash rather than treating every different snapshot as an identity collision.

This ADR supersedes ADR-0001 only for the same-identifier content-mutation and cross-DC conflict semantics above. ADR-0001's Registry Reference representation and exact forward/reverse identity guarantees remain unchanged.
...
* cross-DC tests distinguish valid sequential revisions from divergent same-identity histories.
