...
The division is worth contrasting with the one reservation the registry *does* hold. `cpt-cf-types-registry-fr-registration-authority` refuses `gts.cf.toolkit.*` on the tenant plane because ownership is write-once and a platform contract wrongly admitted as tenant-owned is repairable only by purge — that rule protects a guarantee. A minor on a platform type breaks no guarantee at all; the only thing wrong with it is that it is not how this codebase writes identifiers, and house style belongs in a lint over the source, which fails the author rather than the deployment.
