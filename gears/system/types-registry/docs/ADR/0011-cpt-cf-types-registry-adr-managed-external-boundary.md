...
The common question underneath is what may cross the boundary, in which direction, and what Types Registry is permitted to persist about a crossing. An earlier form of this decision answered "nothing outward, dependents inward, and the inward edges are registered facts". This revision answers the question differently, and the reason is recorded in full below: a registered edge trades dependence on a plugin's *uptime* for dependence on a plugin's *correctness*, and the second is not observable.
...
An earlier form of this ADR carved out one exception — an external identifier held as a label on a dependency edge — on the argument that the set of things depending on a Managed Entity is a fact about that Managed Entity. The argument was sound and the exception is still withdrawn, because closing the boundary removes the thing the exception existed to record. There is no external dependent to label.
...
Chain coverage itself is supplied by the matcher rather than by a separate rule. `gts-id` matches segment-wise and field-wise; a wildcard token replaces a whole token, and once a wildcard segment is reached it accepts every remaining segment including the chain separator. This is the "implicit derived-type coverage" of GTS §3.6 — the behaviour the earlier complete-identifier rule had to work *against*, and which under the closed boundary is exactly what a claim needs.

That earlier rule — a claim matches a complete canonical identifier and never by chain prefix — existed for one reason: to stop a plugin claiming a base type's namespace from silently capturing managed types derived from that base. The protection is unnecessary once managed derivation from an external base is forbidden outright, because there are no managed derivations inside an external claim left to capture.
...
Overlap is decided by the platform matcher's containment primitive, which for rooted single-segment patterns reduces to "one field list is a prefix of the other". A stored upper bound narrows candidates to an index range first; the matcher decides.
...
Live reverse-impact query is retained for **advisory** blast-radius analysis before an incompatible change, where a plugin being unavailable degrades a report rather than corrupting a decision. The distinction is the point, and it survives this revision unchanged: authoritative decisions read local state, informational ones may query.

### A retired Source Claim is a permanent reservation

When a plugin is retired, its Source Claims are retained as **reservations**. Types Registry refuses to register a Managed Entity whose identifier matches a retired claim, and refuses to activate a different plugin's claim over it.

Without this, the same identifier could be re-registered as managed and deterministic Registry References would silently rebind persisted domain references to a different entity.
...
The earlier decision said the opposite: that a vendor "expresses it in its own registry and registers the dependency edge". That sentence is withdrawn. The composition story moves from the external side of the boundary to the managed side, which is where the platform can actually keep its promises about it.

### Consequences

* ADR-0002's enumeration of prohibited persistence is restored in full and acquires no exception. The persistence rule and the enumeration now agree.
* ADR-0007's mandatory P1 capability profile loses dependency registration entirely. Reverse dependency-impact lookup remains optional and advisory. Plugin conformance tests change accordingly.
* **Registry Source Plugins have no write path to Types Registry.** The relationship is read-only, as it was before the asymmetric decision introduced one. The authorization, idempotency, and withdrawal semantics of that path — PRD open question #19 — dissolve rather than being answered.
* ADR-0010's availability-blocking relationship table loses the row for an external subject depending on a managed target. Every remaining blocking relationship holds between two managed entities.
...
* registering a Managed Entity whose identifier matches a retired Source Claim is rejected;
...
- **Storage draft**: [../design-notes/schema-draft.sql](../design-notes/schema-draft.sql)
...
* `cpt-cf-types-registry-fr-registry-source-routing` - defines the rooted single-segment claim grammar and makes a retired claim a permanent reservation.
