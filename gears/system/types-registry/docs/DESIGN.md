...
| `cpt-cf-types-registry-fr-ref-tracking` | Flat managed dependency edges for derivation, `$ref`, and Instance-to-schema relationships; used for deletion safety and impact analysis. `x-gts-ref` is classified but stored as no edge; the managed–external boundary reads that classification from the candidate's content. See *Dependency Graph & Deletion Safety* in §3.2. |
