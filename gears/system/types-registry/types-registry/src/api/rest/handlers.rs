...
pub async fn batch_get_entities(
    ...
    // does not perform.
    let keys: Vec<EntityKey> = req
        .items
        .iter()
        .map(|item| EntityKey::parse(&item.key))
        .collect();
    ...
    // Echo the caller's own spelling rather than re-rendering the parsed key: a
    // round trip through `EntityKey` would hand back a canonicalized string the
    // caller cannot match against what it sent.
    let mut spellings = req.items.into_iter().map(|item| item.key);
    Ok(Json(EntityLookupsDto {
        items: results
            .into_iter()
            .map(|(_, lookup)| EntityLookupDto {
                key: spellings.next().unwrap_or_default(),
                status: (&lookup).into(),
                entity: match lookup {
