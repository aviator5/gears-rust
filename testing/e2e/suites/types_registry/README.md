...
| Path | Covers |
|---|---|
| `test_registration.py` | the asynchronous admission API (`202` + `GET /operations/{id}`), written as numbered scenarios |
...
its scenario document under [`scenarios/`](scenarios/registration.md), and its
JSON input items under `fixtures/`.
...
- Each test owns a namespace: its fixture loader rewrites the topic prefix in
  ...
  included, and changes no schema constraint or value. Today that is
  `registration_fixture` rewriting `cf.e2e.registration.`.
- Every submission carries a fresh `Idempotency-Key`.
- Item outcomes are matched by exact GTS ID, never by position in the response
  array.
